...
from ..user.views import check_access, get_accesses, get_person_by_id


@login_required(login_url='/identity/')
def rfs_home(request):
    from ..admin.models import get_all_portfolio_managers
...
def rfs(request):
    ...
    if request.method == "GET":
        ...
        if 'rfs_request_id' in request.GET:
            ...
            check_access(request.user.username, 'rfs_edit')
    ...
    if request.method == "POST":
        if 'via_view' in request.POST:
            ...
            rfs_status = None
            rfs_status_comment = None
            if 'rfs_status' in request.POST:
                rfs_status = request.POST['rfs_status']
                if 'rfs_status_comment' in request.POST:
                    rfs_status_comment = request.POST['rfs_status_comment']
                models.track_rfs_status(rfs_request_id, rfs_status, request.user.username, rfs_status_comment)

    current_status = models.get_rfs_status(rfs_request_id)
    if current_status and current_status == 'APPROVED':
        latest_version = int(models.get_approval_num(rfs_request_id)) - 1
        rfs_request_data = models.get_approved_rfs_request(rfs_request_id, latest_version)
        rfs_request_scope_data = models.get_approved_rfs_request_scope(rfs_request_id, latest_version)
        rfs_request_ad_data = models.get_approved_rfs_request_ad(rfs_request_id, latest_version)
        pass
    else:
        rfs_request_data = models.get_rfs_request(rfs_request_id)
        rfs_request_scope_data = models.get_rfs_request_scope(rfs_request_id)
        rfs_request_ad_data = models.get_rfs_request_ad(rfs_request_id)
    persons_all = get_all_person()
    approved_rfs = models.get_approved_rfs(rfs_request_id)
    history = models.get_rfs_history(rfs_request_id)
    domains_info = models.get_domains_info()
    user_access = get_accesses(request.user.username)
    approver = check_if_approver(rfs_request_id, request.user.username)
    ...
    editable = False
    if ['NEW', 'REOPENED', 'RETURNED', ''].count(rfs_request_data[0]['rfs_status']):
        editable = True
    ...
    context_data = {
        ...
        'domains_info': domains_info,
        'rfs_request_ad_data': rfs_request_ad_data,
...
def resource_loading(request):
    ...
    if request.method == "POST":
        if 'via_view' in request.POST:
            ...
            for key in request.POST:
                if key == 'f_vnid':
                    request_post[key] = copy(request.POST.getlist(key))
                else:
                    request_post[key] = copy(request.POST[key])
            if 'rfs_request_id' in request.POST:
                rfs_request_id = models.get_rfs_id_from_sha(request.POST['rfs_request_id'])
                current_status = models.get_rfs_status(rfs_request_id)
                if current_status and current_status == 'REOPENED' and request.POST['rfs_status'] == 'SUBMITTED':
                    # if RFS is reopened and now trying to submit again
                    # check if hrs are same
                    # if yes, then put in Submitted status BAU
                    # if not, then auto-approve
                    last_approved_totals = models.get_last_approved_totals(rfs_request_id)
                    print(last_approved_totals)
                    total_qa_hrs = 0
                    for key in request.POST:
                        if key[:10] == 'f_week_hrs' and len(request.POST[key]):
                            total_qa_hrs = total_qa_hrs + float(request.POST[key])
                    if total_qa_hrs == last_approved_totals['rfs_total_hrs']:
                        request_post['rfs_status'] = 'AUTO-APPROVED'
            rfs_request_id = models.update_rfs_resourceLoading(request_post)
            rfs_status = None
            rfs_status_comment = None
            if 'rfs_status' in request_post:
                rfs_status = request_post['rfs_status']
                if 'rfs_status_comment' in request_post:
                    rfs_status_comment = request_post['rfs_status_comment']
                models.track_rfs_status(rfs_request_id, rfs_status, request.user.username, rfs_status_comment)
    ...
    if current_status and current_status == 'APPROVED':
        latest_version = int(models.get_approval_num(rfs_request_id)) - 1
        rfs_resourceLoading_data = models.get_approved_rfs_resourceLoading(rfs_request_id, latest_version)
        rfs_request_data = models.get_approved_rfs_request(rfs_request_id, latest_version)
        static_tbl = models.get_approved_res_load_static(rfs_request_id, latest_version)
    else:
...
def estimates(request):
    ...
    if current_status and current_status == 'APPROVED':
        latest_version = int(models.get_approval_num(rfs_request_id)) - 1
        rfs_estimates = models.get_approved_rfs_estimates(rfs_request_id, latest_version)
        rfs_estimates_raci_head = models.get_approved_rfs_estimates_raci_head(rfs_request_id, latest_version)
        rfs_estimates_raci = models.get_approved_rfs_estimates_raci(rfs_request_id, latest_version)
        rfs_total_rl = models.get_approved_rfs_totals(rfs_request_id, latest_version)
    else:
...
def export_users_xls(request):
    ...
    file = rfs_request_data[0]['project_id'] + " " + project_name + " - Request for Service.xlsx"
    ...
    rfs['B31'] = rfs_request_data[0]['is_sra_completed']
    ...
    rfs['B34'] = rfs_request_data[0]['confidential']
    rfs['B36'] = rfs_request_data[0]['confidential_alt_name']
    rfs['B37'] = rfs_request_data[0]['confidential_doc_share_method']
    rfs['E34'] = rfs_request_data[0]['pii_bsi']
    rfs['E35'] = rfs_request_data[0]['pii_bsi_description']

    rfs['B39'] = rfs_request_data[0]['scope_remote']
    rfs['E39'] = rfs_request_data[0]['scope_ticket_num']
    ...
    for i in range(len(rfs_request_scope_data)):
        ...
        if i == 6:
            if rfs_request_scope_data[i]['include'] == 1:
                rfs['C49'] = 'Yes'
            rfs['D49'] = datetime.strptime(str(rfs_request_scope_data[i]['startdate'])[:10], '%Y-%m-%d').strftime('%m/%d/%Y') if rfs_request_scope_data[i]['startdate'] else ''
            rfs['E49'] = datetime.strptime(str(rfs_request_scope_data[i]['enddate'])[:10], '%Y-%m-%d').strftime('%m/%d/%Y') if rfs_request_scope_data[i]['enddate'] else ''
            rfs['F49'] = rfs_request_scope_data[i]['comment']
    # Updating RFS worksheet End
    # Updating resource loading worksheet start
    if len(rfs_resourse_loading['datadetails']):
        person_id = ''
        row = 4
        col = 5
        ...
        for i in range(len(rfs_resourse_loading['datadetails'])):
            if person_id != rfs_resourse_loading['datadetails'][i]['person_id']:
                ...
                rfs_res_load.cell(row, 61, '=sum(E' + str(row) + ':BH' + str(row) + ')')
                rfs_res_load.cell(row, 63, '=BI' + str(row) + '*BJ' + str(row))
                rfs_res_load.cell(row, 61).border = thin_border
                rfs_res_load.cell(row, 63).border = thin_border
            ...
            rfs_res_load.cell(row, 62, float(rfs_resourse_loading['datadetails'][i]['rate']))
            rfs_res_load.cell(row, 62).number_format = 'General'
            rfs_res_load.cell(row, 62).border = thin_border
    ...
    # Updating estimate worksheet end
    # Updating assumption-dependency start

    print("rfs_ad-------------------------------------->")
    print(rfs_ad)
    if len(rfs_request_ad_data):
        rfs_ad['C8'] = rfs_request_ad_data[0]['val']
        rfs_ad['D8'] = rfs_request_ad_data[0]['domain_name']
        rfs_ad['C9'] = rfs_request_ad_data[1]['val']
        rfs_ad['D9'] = rfs_request_ad_data[1]['domain_name']
        rfs_ad['C10'] = rfs_request_ad_data[2]['val']
        rfs_ad['D10'] = rfs_request_ad_data[2]['domain_name']
        rfs_ad['C11'] = rfs_request_ad_data[3]['val']
        rfs_ad['D11'] = rfs_request_ad_data[3]['domain_name']
        rfs_ad['C12'] = rfs_request_ad_data[4]['val']
        rfs_ad['D12'] = rfs_request_ad_data[4]['domain_name']

        rfs_ad['C16'] = rfs_request_ad_data[5]['val']
        rfs_ad['D16'] = rfs_request_ad_data[5]['domain_name']
        rfs_ad['C17'] = rfs_request_ad_data[6]['val']
        rfs_ad['D17'] = rfs_request_ad_data[6]['domain_name']
        rfs_ad['C18'] = rfs_request_ad_data[7]['val']
        rfs_ad['D18'] = rfs_request_ad_data[7]['domain_name']
        rfs_ad['C19'] = rfs_request_ad_data[8]['val']
        rfs_ad['D19'] = rfs_request_ad_data[8]['domain_name']
        rfs_ad['C20'] = rfs_request_ad_data[9]['val']
        rfs_ad['D20'] = rfs_request_ad_data[9]['domain_name']
...
def export_users_xls_approved(request):
    ...
    rfs['E15'] = datetime.strptime(str(rfs_request_data[0]['tentative_prep_start'])[:10], '%Y-%m-%d').strftime(
        '%m/%d/%Y') if rfs_request_data[0]['tentative_prep_start'] else ''
    rfs['F15'] = datetime.strptime(str(rfs_request_data[0]['tentative_prep_end'])[:10], '%Y-%m-%d').strftime(
        '%m/%d/%Y') if rfs_request_data[0]['tentative_prep_end'] else ''
    rfs['E16'] = datetime.strptime(str(rfs_request_data[0]['tentative_plan_start'])[:10], '%Y-%m-%d').strftime(
        '%m/%d/%Y') if rfs_request_data[0]['tentative_plan_start'] else ''
    rfs['F16'] = datetime.strptime(str(rfs_request_data[0]['tentative_plan_end'])[:10], '%Y-%m-%d').strftime(
        '%m/%d/%Y') if rfs_request_data[0]['tentative_plan_end'] else ''
    rfs['E17'] = datetime.strptime(str(rfs_request_data[0]['tentative_exec_start'])[:10], '%Y-%m-%d').strftime(
        '%m/%d/%Y') if rfs_request_data[0]['tentative_exec_start'] else ''
    rfs['F17'] = datetime.strptime(str(rfs_request_data[0]['tentative_exec_end'])[:10], '%Y-%m-%d').strftime(
        '%m/%d/%Y') if rfs_request_data[0]['tentative_exec_end'] else ''
    rfs['E18'] = datetime.strptime(str(rfs_request_data[0]['tentative_close_start'])[:10], '%Y-%m-%d').strftime(
        '%m/%d/%Y') if rfs_request_data[0]['tentative_close_start'] else ''
    rfs['F18'] = datetime.strptime(str(rfs_request_data[0]['tentative_close_end'])[:10], '%Y-%m-%d').strftime(
        '%m/%d/%Y') if rfs_request_data[0]['tentative_close_end'] else ''
    ...
    for i in range(len(rfs_request_scope_data)):
        ...
        if i == 1:
            if rfs_request_scope_data[i]['include'] == 1:
                rfs['C44'] = 'Yes'
            rfs['D44'] = datetime.strptime(str(rfs_request_scope_data[i]['startdate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['startdate'] else ''
            rfs['E44'] = datetime.strptime(str(rfs_request_scope_data[i]['enddate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['enddate'] else ''
            rfs['F44'] = rfs_request_scope_data[i]['comment']

        if i == 2:
            if rfs_request_scope_data[i]['include'] == 1:
                rfs['C45'] = 'Yes'
            rfs['D45'] = datetime.strptime(str(rfs_request_scope_data[i]['startdate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['startdate'] else ''
            rfs['E45'] = datetime.strptime(str(rfs_request_scope_data[i]['enddate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['enddate'] else ''
            rfs['F45'] = rfs_request_scope_data[i]['comment']
        ...
        if i == 3:
            if rfs_request_scope_data[i]['include'] == 1:
                rfs['C46'] = 'Yes'
            rfs['D46'] = datetime.strptime(str(rfs_request_scope_data[i]['startdate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['startdate'] else ''
            rfs['E46'] = datetime.strptime(str(rfs_request_scope_data[i]['enddate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['enddate'] else ''
            rfs['F46'] = rfs_request_scope_data[i]['comment']
        ...
        if i == 4:
            if rfs_request_scope_data[i]['include'] == 1:
                rfs['C47'] = 'Yes'
            rfs['D47'] = datetime.strptime(str(rfs_request_scope_data[i]['startdate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['startdate'] else ''
            rfs['E47'] = datetime.strptime(str(rfs_request_scope_data[i]['enddate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['enddate'] else ''
            rfs['F47'] = rfs_request_scope_data[i]['comment']
        ...
        if i == 5:
            if rfs_request_scope_data[i]['include'] == 1:
                rfs['C48'] = 'Yes'
            rfs['D48'] = datetime.strptime(str(rfs_request_scope_data[i]['startdate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['startdate'] else ''
            rfs['E48'] = datetime.strptime(str(rfs_request_scope_data[i]['enddate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['enddate'] else ''
            rfs['F48'] = rfs_request_scope_data[i]['comment']
        ...
        if i == 6:
            if rfs_request_scope_data[i]['include'] == 1:
                rfs['C49'] = 'Yes'
            rfs['D49'] = datetime.strptime(str(rfs_request_scope_data[i]['startdate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['startdate'] else ''
            rfs['E49'] = datetime.strptime(str(rfs_request_scope_data[i]['enddate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['enddate'] else ''
            rfs['F49'] = rfs_request_scope_data[i]['comment']
    # Updating RFS worksheet End
            ...
            for i in range(5, 60):
                ...
                if val and len(val):
                    ...
                    if mc_start and (mc_end is None or mc_end < mc_start):
                        mc_end = i - 1
                        # do merge
                        rfs_res_load.cell(row=row + 2, column=mc_start).value = '=sum(' + get_column_letter(
                            mc_start) + str((row + 1)) + ':' + get_column_letter(mc_end) + str((row + 1)) + ')'
                        rfs_res_load.merge_cells(
                            get_column_letter(mc_start) + str(row + 2) + ':' + get_column_letter(mc_end) + str(row + 2))
                        rfs_res_load.cell(row=row + 2, column=mc_start).alignment = Alignment(horizontal='center')
                        mc_start = i
                rfs_res_load.cell(row=row + 2, column=mc_start).border = thick_border
            ...
            if rfs_estimates:
                ...
                for i in range(len(rfs_estimates_raci)):
                    if rfs_estimates_raci[i]['raci_code'] == 'CCQC':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D38'] = str(rfs_estimates_ws['D38'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E38'] = str(rfs_estimates_ws['E38'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D62'] = str(rfs_estimates_ws['D62'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E62'] = str(rfs_estimates_ws['E62'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']

                    if rfs_estimates_raci[i]['raci_code'] == 'TPLPR':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D39'] = str(rfs_estimates_ws['D39'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E39'] = str(rfs_estimates_ws['E39'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D63'] = str(rfs_estimates_ws['D63'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E63'] = str(rfs_estimates_ws['E63'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']

                    if rfs_estimates_raci[i]['raci_code'] == 'TC2QC':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D40'] = str(rfs_estimates_ws['D40'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E40'] = str(rfs_estimates_ws['E40'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D64'] = str(rfs_estimates_ws['D64'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E64'] = str(rfs_estimates_ws['E64'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']

                    if rfs_estimates_raci[i]['raci_code'] == 'EXCTC':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D41'] = str(rfs_estimates_ws['D41'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E41'] = str(rfs_estimates_ws['E41'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D65'] = str(rfs_estimates_ws['D65'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E65'] = str(rfs_estimates_ws['E65'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']

                    if rfs_estimates_raci[i]['raci_code'] == 'MNGQC':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D42'] = str(rfs_estimates_ws['D42'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E42'] = str(rfs_estimates_ws['E42'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D66'] = str(rfs_estimates_ws['D66'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E66'] = str(rfs_estimates_ws['E66'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']

                    if rfs_estimates_raci[i]['raci_code'] == 'CDTER':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D43'] = str(rfs_estimates_ws['D43'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E43'] = str(rfs_estimates_ws['E43'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D67'] = str(rfs_estimates_ws['D67'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E67'] = str(rfs_estimates_ws['E67'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']

                    if rfs_estimates_raci[i]['raci_code'] == 'RDTC':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D44'] = str(rfs_estimates_ws['D44'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E44'] = str(rfs_estimates_ws['E44'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D68'] = str(rfs_estimates_ws['D68'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E68'] = str(rfs_estimates_ws['E68'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']

                    if rfs_estimates_raci[i]['raci_code'] == 'WTR':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D45'] = str(rfs_estimates_ws['D45'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E45'] = str(rfs_estimates_ws['E45'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D69'] = str(rfs_estimates_ws['D69'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E69'] = str(rfs_estimates_ws['E69'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']

                    if rfs_estimates_raci[i]['raci_code'] == 'MXE2E':
                        if rfs_estimates_raci[i]['raci_scope'] == 'SIT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D46'] = str(rfs_estimates_ws['D46'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E46'] = str(rfs_estimates_ws['E46'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                        if rfs_estimates_raci[i]['raci_scope'] == 'UAT':
                            if rfs_estimates_raci[i]['p_or_q'] == 'P':
                                rfs_estimates_ws['D70'] = str(rfs_estimates_ws['D70'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
                            if rfs_estimates_raci[i]['p_or_q'] == 'Q':
                                rfs_estimates_ws['E70'] = str(rfs_estimates_ws['E70'].value) + ' ' + \
                                                          rfs_estimates_raci[i]['RACI']
...
def persons_weeks(request):
    import json
    from ..user.models import get_capacity
    data = {'success': 'False', 'error': '', 'data': [], 'capacity': []}
    if request.method == 'POST':
        records = json.loads(request.POST['data'])
        for rec in records:
            result = models.get_person_week(rec['vnid'], rec['weekstartdt'])
            capacity = get_capacity(rec['vnid'], rec['weekstartdt'], rec['weekstartdt'])
            data['capacity'].append(capacity.copy())
            if len(result):
                total_hrs = 0
                for idx in range(len(result)):
                    total_hrs = total_hrs + float(result[idx]['hrs'])
                summary = {'vnid': rec['vnid'], 'week': rec['weekstartdt'], 'total_hrs': total_hrs}
                data['data'].append({'data': result.copy(), 'summary': summary.copy()})
...
def check_if_approver(rfs_request_id, username):
    ret = False
    if username == 'vn05672':
        return True

    approvers = models.get_approvers(rfs_request_id)
