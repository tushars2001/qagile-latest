...
def check_status_change(rfs_request_id, new_status):
    ret = False

    current_status = get_rfs_status(rfs_request_id)
    if new_status == 'APPROVED':
        ret = True

    if current_status:
        if current_status == 'APPROVED' and new_status == 'REOPENED':
            ret = True
        if current_status == 'SUBMITTED' and new_status == 'RETURNED':
            ret = True
...
def update_rfs_request(postdata):
    ...
    isOnlyStatusChange = check_status_change(fields['rfs_request_id'], fields['rfs_status'])
    ...
    fields['is_sra_completed'] = ifnotdef(postdata, 'is_sra_completed', 'str')
    ...
    if not exist:
        ...
        sql = """update qagile_rfs_request 
                        set 
                            ...
                            `is_sra_completed`	=	%(is_sra_completed)s ,
                            `qa_spoc`	=	%(qa_spoc)s ,
...
def update_rfs_resourceLoading(postdata):
    ...
    with connection.cursor() as cursor:
        ...
        for sql in deatilsqls:
            cursor.execute(sql, fields)
        cursor.close()
    print(head_sql, deatilsqls, del_detail_sql)
    set_rfs_status(fields['rfs_request_id'], fields['rfs_status'])
    return fields['rfs_request_id']
