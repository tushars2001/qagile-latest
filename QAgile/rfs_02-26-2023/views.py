...
def export_users_xls_approved(request):
    ...
    for i in range(len(rfs_request_scope_data)):
        ...
        if i == 1:
            if rfs_request_scope_data[i]['include'] == 1:
                rfs['C43'] = 'Yes'
            rfs['D43'] = datetime.strptime(str(rfs_request_scope_data[i]['startdate'])[:10], '%Y-%m-%d').strftime(
                '%m/%d/%Y') if rfs_request_scope_data[i]['startdate'] else ''
            rfs['E43'] = datetime.strptime(str(rfs_request_scope_data[i]['enddate'])[:10], '%Y-%m-%d').strftime(
